...
public class Client
{
	...
	public Question requestQuestion()
	{
		...
		try {
			ObjectInputStream objectInput = new ObjectInputStream(socket.getInputStream()); //prepare object stream
			object = objectInput.readObject();	//read received object
		} catch (Exception e) {
			e.printStackTrace();
		}
		return (Question) object;	//convert object to Question object before returning
	}
	...
	public Vector<String> requestUsers()
	{
		...
		Vector<String> users = new Vector<String>();	//prepare vector of usernames
		try {
			ObjectInputStream objectInput = new ObjectInputStream(socket.getInputStream()); //prepare object stream
			Object object = objectInput.readObject();	//read received vector
			users = (Vector<String>) object;		//convert object to vector
		} catch (Exception e) {
			e.printStackTrace();
		}
		finally {
			return users;	//return vector of usernames to GUI
		}
	}
}
